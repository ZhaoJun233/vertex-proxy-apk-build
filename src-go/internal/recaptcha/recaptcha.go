...
func FetchRecaptchaToken(net *transport.NetworkClient, proxyURI string) (string, error) {
	...
	for retry := 0; retry < 3; retry++ {
		...
		if token, ok := fetchOnce(net, proxyURI, retry+1); ok {
...
func fetchOnce(net *transport.NetworkClient, proxyURI string, attempt int) (string, bool) {
	sess, err := net.CreateSession(15, proxyURI, "recaptcha")
	if err != nil {
		log.Printf("[Recaptcha] attempt %d create session failed, proxy=%q, err=%v", attempt, proxyURI, err)
		return "", false
	}
	defer sess.Close()
	...
	if err != nil {
		log.Printf("[Recaptcha] attempt %d anchor request failed, proxy=%q, err=%v", attempt, proxyURI, err)
		return "", false
	}
	...
	if err != nil {
		log.Printf("[Recaptcha] attempt %d reload request failed, proxy=%q, err=%v", attempt, proxyURI, err)
		return "", false
	}
	if status != 200 {
	...
	if rm == nil {
		...
		log.Printf("[Recaptcha] attempt %d reload token parse failed, body prefix: %s", attempt, bodyStr)
		return "", false
	}
